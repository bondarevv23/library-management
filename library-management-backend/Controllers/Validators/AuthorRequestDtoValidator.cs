...
public class AuthorRequestDtoValidator : AbstractValidator<AuthorRequestDto>
{
    public AuthorRequestDtoValidator()
    {
        RuleFor(request => request.Name)
            ...
            .NotNull()
            .WithMessage($"{BODY_NAME_FILED} {NOT_NULL_CONSTRAINT}")
        ...
        RuleFor(request => request.DateOfBirth)
            .Cascade(CascadeMode.Stop)
            .NotNull()
            .WithMessage($"{BODY_DATE_OF_BIRTH_FIELD} {NOT_NULL_CONSTRAINT}")
            .Must(BeValidDateOnly)
            .WithMessage($"{BODY_DATE_OF_BIRTH_FIELD} must be valid DateOnly");
    }
}
